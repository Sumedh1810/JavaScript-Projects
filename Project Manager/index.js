...
crud.getAllTasks().forEach((tasks, index) => {
    tasks.forEach(task => { 
...
addForm.forEach(form => {
    form.addEventListener("submit", event => {
        ...
        if(form.task.value){
            const task = crud.insertTask(form.submit.dataset.id, form.task.value.trim());
...
taskbox.forEach(column => {
    column.addEventListener("click", event => {
        ...
        if(event.target.classList.contains("delete")){
            ...
            crud.deleteTask(event.target.dataset.id);
        }
